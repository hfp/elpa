...
function check_correctness_evp_numeric_residuals_ss_real_&
                                               &PRECISION&
                                               &(na, nev, as, z, ev, sc_desc, nblk, myid, &
                                                 np_rows, np_cols, my_prow, my_pcol) result(status)
...
function check_correctness_evp_numeric_residuals_ss_real_&
                                               &PRECISION&
                                               &_f (na, nev, na_rows, na_cols, as, z, ev, sc_desc, nblk, &
                                                    myid, np_rows, np_cols, my_prow, my_pcol) result(status) &
...
#include "./test_precision_kinds.F90"
  ...
  allocate(tmp1(size(as,dim=1), size(as,dim=2)))
  allocate(tmp2(size(as,dim=1), size(as,dim=2)))
...
#include "./test_precision_kinds.F90"
  ...
  real(kind=rk)               :: ev(na)
  real(kind=rk), allocatable  :: ev_analytic(:)
...
#include "./test_precision_kinds.F90"
  ...
  real(kind=rk), allocatable                                        :: tmp1_real(:,:)
#endif
  ...
  allocate(tmp2(size(as,dim=1), size(as,dim=2)))
#if COMPLEXCASE == 1
  allocate(tmp1_real(2*size(as,dim=1), size(as,dim=2)))
#endif
...
#include "./test_precision_kinds.F90"
  ...
  allocate(tmp1(size(a,dim=1), size(a,dim=2)))
  allocate(tmp2(size(a,dim=1), size(a,dim=2)))
#if COMPLEXCASE == 1
  allocate(tmp1_real(2*size(a,dim=1), size(a,dim=2)))
#endif
