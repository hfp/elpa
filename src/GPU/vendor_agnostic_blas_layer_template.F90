...
! This file is the generated version. Do NOT edit
  ...
  contains
    ...
    subroutine gpublas_Dtrmv_intptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
      use, intrinsic :: iso_c_binding
      use cuda_functions
#ifdef WITH_AMD_GPU_VERSION
      use hip_functions
      ...
      integer(kind=c_intptr_t), value :: gpublasHandle

        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Dtrmv_intptr  (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Dtrmv_intptr (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Dtrmv_intptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Dtrmv_cptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
      use, intrinsic :: iso_c_binding
      use cuda_functions
#ifdef WITH_AMD_GPU_VERSION
      use hip_functions
      ...
      integer(kind=c_intptr_t), value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Dtrmv_cptr  (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Dtrmv_cptr (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Dtrmv_cptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif
    end subroutine
    subroutine gpublas_Strmv_intptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
      use, intrinsic :: iso_c_binding
      use cuda_functions
#ifdef WITH_AMD_GPU_VERSION
      use hip_functions
      ...
      integer(kind=c_intptr_t), value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Strmv_intptr  (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Strmv_intptr (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Strmv_intptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Strmv_cptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
      use, intrinsic :: iso_c_binding
      use cuda_functions
#ifdef WITH_AMD_GPU_VERSION
      use hip_functions
      ...
      integer(kind=c_intptr_t), value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Strmv_cptr  (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Strmv_cptr (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Strmv_cptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif
    end subroutine
    subroutine gpublas_Ztrmv_intptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
      use, intrinsic :: iso_c_binding
      use cuda_functions
#ifdef WITH_AMD_GPU_VERSION
      use hip_functions
      ...
      integer(kind=c_intptr_t), value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Ztrmv_intptr  (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Ztrmv_intptr (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Ztrmv_intptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Ztrmv_cptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
      use, intrinsic :: iso_c_binding
      use cuda_functions
#ifdef WITH_AMD_GPU_VERSION
      use hip_functions
      ...
      integer(kind=c_intptr_t), value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Ztrmv_cptr  (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Ztrmv_cptr (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Ztrmv_cptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif
    end subroutine
    subroutine gpublas_Ctrmv_intptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
      use, intrinsic :: iso_c_binding
      use cuda_functions
#ifdef WITH_AMD_GPU_VERSION
      use hip_functions
      ...
      integer(kind=c_intptr_t), value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Ctrmv_intptr  (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Ctrmv_intptr (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Ctrmv_intptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Ctrmv_cptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
      use, intrinsic :: iso_c_binding
      use cuda_functions
#ifdef WITH_AMD_GPU_VERSION
      use hip_functions
      ...
      integer(kind=c_intptr_t), value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Ctrmv_cptr  (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Ctrmv_cptr (uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Ctrmv_cptr(uplo, trans, diag, n, a, lda, x, incx, gpublasHandle)
        endif
#endif
    end subroutine
    subroutine gpublas_Dsyrk_intptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
      ...
      integer(kind=c_intptr_t) , value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Dsyrk_intptr  (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Dsyrk_intptr (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Dsyrk_intptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Dsyrk_cptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
      ...
      integer(kind=c_intptr_t) , value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Dsyrk_cptr  (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Dsyrk_cptr (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Dsyrk_cptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Ssyrk_intptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
      ...
      integer(kind=c_intptr_t) , value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Ssyrk_intptr  (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Ssyrk_intptr (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Ssyrk_intptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Ssyrk_cptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
      ...
      integer(kind=c_intptr_t) , value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Ssyrk_cptr  (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Ssyrk_cptr (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Ssyrk_cptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Zherk_intptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
      ...
      integer(kind=c_intptr_t) , value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Zherk_intptr  (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Zherk_intptr (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Zherk_intptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Zherk_cptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
      ...
      integer(kind=c_intptr_t) , value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Zherk_cptr  (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Zherk_cptr (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Zherk_cptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Cherk_intptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
      ...
      integer(kind=c_intptr_t) , value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Cherk_intptr  (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Cherk_intptr (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Cherk_intptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif
    end subroutine

    subroutine gpublas_Cherk_cptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
      ...
      integer(kind=c_intptr_t) , value :: gpublasHandle
        ...
        if (use_gpu_vendor == nvidia_gpu) then
          call cublas_Cherk_cptr  (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif

#ifdef WITH_AMD_GPU_VERSION
        if (use_gpu_vendor == amd_gpu) then
          call rocblas_Cherk_cptr (uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif

#ifdef WITH_SYCL_GPU_VERSION
        if (use_gpu_vendor == sycl_gpu) then
          call syclblas_Cherk_cptr(uplo, trans, n, k, alpha, a, lda, beta, c, ldc, gpublasHandle)
        endif
#endif
    end subroutine
